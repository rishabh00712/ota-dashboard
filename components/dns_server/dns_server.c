...
void dns_server_task(void *pvParameters)
{
    ...
    while (handle->started)
    {
        ...
        if (err < 0)
        {
            ...
            shutdown(sock, 0); // cleanup
            close(sock);
            sock = -1;
            vTaskDelay(pdMS_TO_TICKS(100)); // Give time before retrying
...
void stop_dns_server(dns_server_handle_t handle)
{
    if (handle)
    {
        ...
        if (handle->task != xTaskGetCurrentTaskHandle())
        {
            // Instead of deleting and freeing blindly...
            // Wait or signal the task to shut itself down cleanly.
            ESP_LOGW(TAG, "Not current task. Posting shutdown signal instead of vTaskDelete.");
        }
        else
        {
            // The DNS task itself should do the cleanup:
            handle->task = NULL;
        }
    }
}
